  ...
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        // Use central utility for the game fetch to handle fallbacks
        const [game, grData, cData] = await Promise.all([
          fetchRespectGame(resolvedParams.id),
          safeFetch(`${API_BASE_URL}/respect/grade-levels`),
          safeFetch(`${API_BASE_URL}/respect/courses`),
        ]);

        if (grData?.data) setGradeLevels(grData.data);
        ...
        if (game) {
          setForm({
            ...
            points_reward:            game.points_reward      ?? 10,
            time_limit:               game.time_limit         ?? '',
            order_index:              game.order_index        ?? 0,
            is_active:                game.is_active          ?? true,
            is_free:                  game.is_free            ?? true,
            enable_instruction_audio: game.enable_instruction_audio ?? true,
            game_data:                game.game_data          ?? {},
          });
        } else {
          setError('Quest not found in cloud registry or access denied.');
        }
      } catch (e) {
        console.error(e);
        setError('Failed to sync quest data from Learning Cloud.');
      } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/respect/games/${resolvedParams.id}`,
        {
          method:  'PUT',
      ...
      console.error(err);
      setError(err.message || 'Failed to update game on Learning Cloud.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-24">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-1">
            <span className="px-3 py-1 bg-amber-100 text-amber-600 rounded-full text-[0.6rem] font-black uppercase tracking-widest">
              Editing
            </span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">
            {form.title || 'Game Activity'}
          </h1>
          <p className="text-slate-400 font-semibold font-serif italic mt-1">
            Update this RESPECT game activity.
          </p>
        </div>

        {/* Quick preview link */}
        <Link
          href={`/respect-minimal-games/games/${resolvedParams.id}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-5 py-3 bg-primary/10 text-primary rounded-2xl font-bold text-sm hover:bg-primary hover:text-white transition-all"
        >
          <Gamepad2 size={16} />
          Preview
        </Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* ── Section 2: Game Type ── */}
        <Section icon={<Gamepad2 size={20} />} title="Game Type" dark>
                  ...
                  <button
                    ...
                    className={`py-3 px-3 rounded-2xl font-bold text-xs transition-all border-2 text-center leading-tight ${
                      isSelected
                        ...
                        : 'bg-white border-slate-700/50 text-slate-400 hover:border-primary/40 hover:text-primary'
                    }`}
                  >
                    {label}
                  </button>
        ...
        <Section icon={<Star size={20} />} title="Difficulty Level">
          ...
          >
            <div className="space-y-4">
              ...
              <DynamicGameDataEditor 
                ...
                gameData={form.game_data}
                onChange={(gd) => set('game_data', gd)}
              />
            </div>
          </Section>
        )}
        ...
        <Section icon={<Settings size={20} />} title="Game Settings">
          <div className="space-y-5">
        ...
        {/* ── Section 6: Access & Flags ── */}
        <Section icon={<Globe size={20} />} title="Access & Flags">
        ...
        <div className="flex gap-4 pt-2">
          ...
          >
            {success ? (
              <><CheckCircle2 size={20} /> Updated — Redirecting...</>
            ) : submitting ? (
...
function Section({
  ...
  return (
    <div className={`p-8 rounded-[3rem] border space-y-6 ${
